...
void als(double *x, int group) {
  ...
  double B[TA*TB*TC]; // considered as (TY*TZ) x TX matrix in col major order
  for (int i=0; i<TA; ++i) {
    for (int j=0; j<TB; ++j) {
      for (int k=0; k<TC; ++k) {
        B[(s(0,i,j,k)*s(1,TA,TB,TC) + s(1,i,j,k))*s(2,TA,TB,TC) + s(2,i,j,k)] = T[i][j][k];
      }
    }
  }
  double A[max(max(TA*TB*SC,TA*SB*TC),SA*TB*TC)];
  memset(A,0,s(0,SA,SB,SC)*s(1,TA,TB,TC)*s(2,TA,TB,TC)*sizeof(double));
  for (int t=0; t<SNZ; ++t) {
    for (int j=0; j<s(1,TA,TB,TC); ++j) {
      for (int k=0; k<s(2,TA,TB,TC); ++k) {
        ...
        A[(s(0,SI[t],SJ[t],SK[t])*s(1,TA,TB,TC)+j)*s(2,TA,TB,TC)+k] += cur;
      }
    }
  }
  int m = s(2,TA,TB,TC)*s(1,TA,TB,TC);
  int n = s(0,SA,SB,SC);
